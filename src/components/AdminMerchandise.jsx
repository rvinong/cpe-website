    ...
    <div className="admin-page mx-auto max-w-7xl">
      ...
      <section className="mt-7 overflow-hidden rounded-2xl border border-slate-200 bg-white">
        ...
        {isLoading ? <div className="p-7 text-sm font-bold text-slate-500">Loading merchandise workspace...</div> : activeTab === 'products' ? (
          ...
          filteredOrders.length === 0 ? <div className="p-7 text-sm font-bold text-slate-500">No merchandise orders have been submitted yet.</div> : <div className="divide-y divide-slate-200">{filteredOrders.map((order) => <article key={order.id} className="px-5 py-5 sm:px-6"><div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="text-xs font-extrabold tracking-[0.16em] text-brand-600 uppercase">{order.order_number}</span><span className={`rounded-full px-2.5 py-1 text-[10px] font-extrabold ring-1 ${orderStatusStyles[order.status] || orderStatusStyles.pending}`}>{order.status}</span></div><h3 className="mt-2 text-lg font-black text-navy-900">{order.customer_name}</h3><p className="mt-1 text-xs text-slate-500">{order.customer_email} · {order.contact_number} · {formatMerchandiseOrderDate(order.created_at)}</p></div><label className="text-xs font-extrabold text-slate-500">Update status<select className="admin-field mt-2 min-w-48" value={order.status} disabled={updatingOrderId === order.id} onChange={(event) => handleOrderStatusChange(order, event.target.value)}>{orderStatusOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div><div className="mt-5 grid gap-3 border-t border-slate-100 pt-4 sm:grid-cols-[1fr_auto] sm:items-end"><div className="space-y-2">{order.items.map((item) => <div key={item.id} className="flex justify-between gap-4 text-sm"><span className="text-slate-600">{item.quantity} x {item.product_name} ({item.variant_size})</span><span className="font-extrabold text-navy-900">{formatMerchandisePrice(item.line_total)}</span></div>)}<p className="pt-2 text-xs font-bold text-slate-400">{order.fulfillment_method === 'pickup' ? 'Campus pickup' : `Delivery: ${order.delivery_address}`}; {order.payment_method.replaceAll('_', ' ')}</p></div><div className="text-left sm:text-right"><p className="text-xs font-extrabold tracking-wide text-slate-400 uppercase">Order total</p><p className="mt-1 text-2xl font-black text-navy-900">{formatMerchandisePrice(order.subtotal)}</p></div></div></article>)}</div>
        )}
      </section>
