export const alumniProfiles = [
  {
    id: 'sample-alumni-mikaela-santos',
    name: 'Sample Alumni Mikaela Santos',
    batch: '2022',
    role: 'Software QA Analyst',
    professional_role: 'Software QA Analyst',
    organization: 'Visayas Tech Solutions',
    history:
      'Former documentation lead who helped organize technical review sessions and student project showcases.',
    organization_history:
      'Former documentation lead who helped organize technical review sessions and student project showcases.',
    highlight:
      'Supports junior students by sharing testing workflows, portfolio tips, and early-career software engineering lessons.',
    photo: null,
    photo_path: null,
    initials: 'MS',
    featured: true,
    is_featured: true,
    status: 'published',
    consent_confirmed: true,
    sort_order: 10,
    published_at: '2026-06-27T08:00:00+08:00',
  },
  {
    id: 'sample-alumni-carlo-dela-cruz',
    name: 'Sample Alumni Carlo Dela Cruz',
    batch: '2021',
    role: 'Network Support Engineer',
    professional_role: 'Network Support Engineer',
    organization: 'Regional IT Services',
    history:
      'Served as a technical committee volunteer for laboratory support and peer troubleshooting activities.',
    organization_history:
      'Served as a technical committee volunteer for laboratory support and peer troubleshooting activities.',
    highlight:
      'Works with campus-style network deployments and encourages students to build practical troubleshooting habits.',
    photo: null,
    photo_path: null,
    initials: 'CD',
    featured: false,
    is_featured: false,
    status: 'published',
    consent_confirmed: true,
    sort_order: 20,
    published_at: '2026-06-27T08:00:00+08:00',
  },
]
